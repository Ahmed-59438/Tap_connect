...
def upgrade() -> None:
    """Upgrade schema."""
    # Drop old connections table if it exists (from previous non-modular attempts)
    # We do this to ensure a clean schema matching our new model.
    try:
        op.drop_table('connections')
    except Exception:
        pass # Ignore if it doesn't exist or fails (we'll try to create anyway)
